function moveFrameFunc(state, setState, that) {
    return function(e) {
        function checkPosition(tar) {
            if (indexR) {
                for (let i = 0; i < fWA.length; i++) {
                    if (indexR == fWA.length - 1) {
                        fWA[i].style.marginBottom = `${fWA[i].offsetHeight +
                            fWA[i].offsetHeight / 10}px`;
            ...
            for (let i = 0; i < fWA.length; i++) {
                if (
                    tar > fWA[i].offsetTop - fWA[i].offsetHeight / 4 &&
                    tar < fWA[i].offsetTop + fWA[i].offsetHeight / 4
                ) {
                    fWA[i].style.marginTop = `${fWA[i].offsetHeight +
                        fWA[i].offsetHeight / 10}px`;
        ...
        function msUp() {
            ...
            if (sibler) {
                if (
                    fWA.indexOf(sibler) == fWA.length - 1 &&
                    sibler.getAttribute("data-last") == "true"
                ) {
                    let newArr = state.frames.splice(
                        aim.getAttribute("data-position") - 1,
                        1
                    );
                    ...
                    state.frames.splice(
                        +sibler.getAttribute("data-position") - 1,
                        0,
                        newArr[0]
                    );
                    that.setState(state);
        ...
        function msMove(e) {
            let coorY =
                e.pageY -
                (aim.offsetHeight - aim.offsetHeight / 10) -
                headerHeight;
