function penToolFunc(state, setState, that) {
    function penTool(e) {
        ...
        let pixelSize = Math.ceil(
            canvas.scrollWidth / state.canvasSettings.rows
        );
        ...
        function msMove(e) {
            let x = Math.floor(
                e.offsetX / (canvas.scrollWidth / state.canvasSettings.rows)
            );
            let y = Math.floor(
                e.offsetY / (canvas.scrollWidth / state.canvasSettings.columns)
            );
            if (
                x < 0 ||
                x > state.canvasSettings.rows - 1 ||
                y < 0 ||
                y > state.canvasSettings.columns - 1
            ) {
                return;
            }
            switch (penWidth) {
                ...
                case 2:
                    ...
                    state.frames[state.currFrame].matrix[x - 1][
                        y - 1
                    ] = curColor;
                    break;
                case 3:
                ...
                case 4:
                    ...
                    state.frames[state.currFrame].matrix[x - 2][
                        y + 1
                    ] = curColor;
                    state.frames[state.currFrame].matrix[x - 2][
                        y - 1
            ...
            for (
                let i = 0;
                i < state.frames[state.currFrame].matrix.length;
                i++
            ) {
                for (
                    let q = 0;
                    q < state.frames[state.currFrame].matrix[i].length;
                    q++
                ) {
